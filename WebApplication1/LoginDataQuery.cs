...
namespace MetricsAPI
{
    public class LoginDataQuery
    {
        ...
        public LoginDataQuery(AppDb db)
        {
            Db = db;
        }
        ...
        public async Task<LoginData> FindOneAsync(int id)
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT Record, Time_Stamp, AccountName, AccountType FROM Logins WHERE Record = @Record;";
        ...
        /// </summary>
        /// <returns></returns>
        public async Task<List<LoginData>> LatestPostsAsync()
        {
            using var cmd = Db.Connection.CreateCommand();
            cmd.CommandText = @"SELECT Record, Time_Stamp, AccountName, AccountType FROM Logins ORDER BY Record;";
        ...
        /// <returns></returns>
        private async Task<List<LoginData>> ReadAllAsync(DbDataReader reader)
        {
